...
function task2() {
  try {
    console.log(myVariable); // myVariable ei ole määritelty -> ReferenceError
  } catch (error) {
    if (error instanceof ReferenceError) {
      console.log("ReferenceError havaittu: " + error.message);
    } else {
      console.log("Jokin muu virhe: " + error.message);
    }
  }
}
...
function task3() {
  try {
    notDefinedFunction(); // Simuloitu virhe: funktiota ei ole olemassa
  } catch (error) {
    console.log("Virhe havaittu: " + error.message);
  } finally {
...
// Testitapaukset
console.log(parseJSON('{"name": "Alice", "age": 25}')); // Palauttaa olion { name: 'Alice', age: 25 }
...
function task9(malformedURI) {
  try {
    ...
    if (error instanceof URIError) {
      console.log("Virhe URIError: virheellinen URI");
    } else {
      console.log("Muu virhe:", error.message);
    }
  }
}
