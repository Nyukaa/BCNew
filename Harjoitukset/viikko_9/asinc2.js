...
function processOrderPromise() {
  return wait(2000, "Tilaus vastaanotettu")
    .then((msg1) => {
      console.log(msg1);
      return wait(3000, "Tilausta valmistellaan");
    })
    .then((msg2) => {
      console.log(msg2);
      return wait(1000, "Tilaus valmis noudettavaksi");
    });
  // .then((msg3) => {
  //   console.log(msg3);
  //   return msg3;
...
function getUserProfile(userExists) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (userExists) {
        reject("Käyttäjäprofiili ladattu");
      } else {
        resolve("Käyttäjää ei löytynyt");
      }
    }, 2000);
  });
}
