...
function next_saturday_tour() {
    $today = current_time('Y-m-d');
    $dayOfWeek = date('w', strtotime($today));
    $daysUntilSaturday = (6 - $dayOfWeek + 7) % 7;
    if ($daysUntilSaturday === 0) $daysUntilSaturday = 7;
    $nextSaturday = date('d.m', strtotime("+$daysUntilSaturday days"));
    
    return '<div class="next-tour"> <strong>' . $nextSaturday . ' is next tour</strong></div>';
}
add_shortcode('next_tour', 'next_saturday_tour');
